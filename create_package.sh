...
#datahub_webapp_full_filename
datahub_webapp_filename=datahub-webapp-6.6.0.1-RC2.war
...
datahub_webapp_dir=$hybris_dir/hybris/bin/ext-integration/datahub/web-app
datagub_webapp_file=$datahub_webapp_dir/$datahub_webapp_filename
...
package_datahub_dir=$package_dir/datahub
package_datahub_bin_dir=$package_datahub_dir/bin
package_datahub_bin_file=$package_datahub_bin_dir/datahub-webapp.war
package_datahub_config_dir=$package_datahub_dir/config
...
if [ ! -d "$package_datahub_bin_dir" ]; then
  mkdir -p "$package_datahub_bin_dir"
fi
if [ ! -d "$package_datahub_config_dir" ]; then
  mkdir -p "$package_datahub_config_dir"
fi
...
if [ "$BUILD" = true ] ; then
  ...
  echo "omitting ant production command"
fi
...
echo "copy data hub file"
yes | cp -rf $datagub_webapp_file $package_datahub_bin_file
